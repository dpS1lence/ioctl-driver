...
namespace driver {
	...
	template <class T>
	void wtire_memory(HANDLE driver_handle, const std::uintptr_t addr, const T& value) {
		Request r;
		r.target = reinterpret_cast<PVOID>(addr);
...
int main() {
	...
	const HANDLE driver = CreateFile(L"\\\\.\\Driver", GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
